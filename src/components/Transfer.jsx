...
export default function Transfer({ wallets, onTransfer }) {
  ...
  const validate = () => {
    ...
    else if (type === 'send' && p > wallet.balance)
      e.amount = `Insufficient — max ${fmt(wallet.balance)} ${asset}`
  ...
  if (success)
    return (
      <div
        data-testid='screen-transfer-success'
        className={`${styles.successRoot} fade-up`}
      >
        <div className={styles.successCard}>
          ...
          <div data-testid='success-summary' className={styles.summaryBox}>
            {[
              ...
              {
                label: 'Amount',
                value: `${fmt(success.amount)} ${success.asset}`,
                testid: 'success-amount',
                mono: true,
              },
              {
                label: 'USD',
                value: fmtUSD(success.usdValue),
                testid: 'success-usd',
              },
              {
                label: 'Tx ID',
              ...
              <div key={r.testid} className={styles.summaryRow}>
                ...
                <span
                  ...
                  className={`${styles.summaryValue} ${r.mono ? 'mono' : ''}`}
                >
                  {r.value}
                </span>
              </div>
    ...
    <div data-testid='screen-transfer' className={`${styles.root} fade-up`}>
      <div className={styles.inner}>
        ...
        <form onSubmit={handleSubmit} data-testid='form-transfer' noValidate>
          ...
          >
            {wallets.map((w) => (
              <option key={w.symbol} value={w.symbol}>
                {w.icon} {w.name} ({w.symbol}) — {fmt(w.balance)} available
              </option>
            ))}
          </select>
          ...
          {errors.amount && (
            <p data-testid='error-amount' className={styles.errorMsg}>
              {errors.amount}
            </p>
          )}
          {amount && !isNaN(parseFloat(amount)) && (
            <p data-testid='usd-preview' className={styles.usdPreview}>
              ≈ {fmtUSD(usdPreview)}
            </p>
          )}
          ...
          {type === 'send' && (
            <>
              ...
              />
              {errors.address && (
                <p data-testid='error-address' className={styles.errorMsg}>
                  {errors.address}
                </p>
              )}
            </>
          ...
          {type === 'receive' && wallet && (
            <div data-testid='receive-address' className={styles.receiveBox}>
              <p className={styles.receiveLabel}>
                Your {asset} deposit address
              </p>
              <p
                data-testid='receive-address-value'
                className={`${styles.receiveAddr} mono`}
              >
                {wallet.address}
              </p>
              <button
